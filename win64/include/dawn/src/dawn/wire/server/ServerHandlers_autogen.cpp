...
namespace dawn::wire::server {
    ...
    bool Server::HandleDeviceGetAdapter(DeserializeBuffer* deserializeBuffer) {
        ...
        auto* resultData = AdapterObjects().Allocate(cmd.result.id);
        if (resultData == nullptr) {
            return false;
        }
        resultData->generation = cmd.result.generation;

        bool success = DoDeviceGetAdapter(cmd.self, &resultData->handle);
        ...
        while (deserializeBuffer.AvailableSize() >= sizeof(CmdHeader) + sizeof(WireCmd)) {
            ...
            switch (cmdId) {
                ...
                case WireCmd::DeviceGetAdapter:
                    success = HandleDeviceGetAdapter(&deserializeBuffer);
                    break;
