...
size_t DeviceGetAdapterCmd::GetRequiredSize() const {
    size_t size = sizeof(DeviceGetAdapterTransfer) + DeviceGetAdapterGetExtraRequiredSize(*this);
    return size;
}
...
WireResult DeviceGetAdapterCmd::Serialize(
    ...
    WIRE_TRY(buffer->Next(&transfer));
    transfer->commandSize = commandSize;
    return (DeviceGetAdapterSerialize(*this, transfer, buffer, provider));
}
WireResult DeviceGetAdapterCmd::Serialize(size_t commandSize, SerializeBuffer* buffer) const {
    ErrorObjectIdProvider provider;
    return Serialize(commandSize, buffer, provider);
}
